...
class AdminNavBar(BoxLayout):
    TABS = [("Dashboard", "📊"), ("Apps", "📦"), ("News", "📰"), ("Updates", "🔔"), ("Exit", "🚪")]
    ...
    def _pressed(self, name):
        if name == "Exit":
            ...
            return
        mapping = {"Dashboard": "admin_dashboard", "Apps": "admin_apps", "News": "admin_news", "Updates": "admin_updates"}
        if name in mapping:
...
class AdminScreenWithNav(Screen):
    nav_tab = StringProperty("Dashboard")
...
class AdminLoginScreen(Screen):
    def __init__(self, **kw):
        super().__init__(**kw)
        self.store = JsonStore("admin_auth.json")
        self._build()
...
class AppFormScreen(Screen):
    ...
    def _build(self):
        ...
        layout.add_widget(field_label("App Name *")); self.f_name = SHVTextInput(text=item.get("name","")); layout.add_widget(self.f_name)
        layout.add_widget(field_label("Tagline")); self.f_tagline = SHVTextInput(text=item.get("tagline","")); layout.add_widget(self.f_tagline)
        layout.add_widget(field_label("Version *")); self.f_version = SHVTextInput(text=item.get("version","")); layout.add_widget(self.f_version)
        layout.add_widget(field_label("Category")); self.f_category = SHVTextInput(text=item.get("category","Utility")); layout.add_widget(self.f_category)
        layout.add_widget(field_label("Package Name")); self.f_package = SHVTextInput(text=item.get("package_name","")); layout.add_widget(self.f_package)
        layout.add_widget(spacer(4)); layout.add_widget(section_label("Description"))
        self.f_desc = SHVMultiInput(text=item.get("description",""), height=dp(140)); layout.add_widget(self.f_desc)
        layout.add_widget(spacer(4)); layout.add_widget(section_label("URLs"))
        layout.add_widget(field_label("APK Download URL")); self.f_apk = SHVTextInput(text=item.get("apk_url","")); layout.add_widget(self.f_apk)
        layout.add_widget(field_label("Icon URL")); self.f_icon = SHVTextInput(text=item.get("icon_url","")); layout.add_widget(self.f_icon)
...
class NewsFormScreen(Screen):
    ...
    def _build(self):
        ...
        layout.bind(minimum_height=layout.setter("height"))
        self.status = StatusBar(size_hint_x=1); layout.add_widget(self.status)
        layout.add_widget(field_label("Title *")); self.f_title = SHVTextInput(text=item.get("title","")); layout.add_widget(self.f_title)
        layout.add_widget(field_label("Body *")); self.f_body = SHVMultiInput(text=item.get("body",""), height=dp(200)); layout.add_widget(self.f_body)
        layout.add_widget(field_label("Cover Image URL")); self.f_cover = SHVTextInput(text=item.get("cover_image_url","")); layout.add_widget(self.f_cover)
        pub_row = BoxLayout(size_hint_y=None, height=dp(44), spacing=dp(12))
        pub_row.add_widget(SHVLabel(text="Published", size_hint_y=None, height=dp(44)))
...
class UpdateFormScreen(Screen):
    ...
    def _build(self):
        ...
        layout.bind(minimum_height=layout.setter("height"))
        self.status = StatusBar(size_hint_x=1); layout.add_widget(self.status)
        layout.add_widget(field_label("App Name *")); self.f_appname = SHVTextInput(text=item.get("app_name","")); layout.add_widget(self.f_appname)
        layout.add_widget(field_label("Feature Title *")); self.f_title = SHVTextInput(text=item.get("title","")); layout.add_widget(self.f_title)
        layout.add_widget(field_label("Description")); self.f_desc = SHVMultiInput(text=item.get("description",""), height=dp(120)); layout.add_widget(self.f_desc)
        layout.add_widget(field_label("Target Version")); self.f_version = SHVTextInput(text=item.get("target_version","")); layout.add_widget(self.f_version)
        layout.add_widget(field_label("Expected Date")); self.f_eta = SHVTextInput(text=item.get("expected_date","")); layout.add_widget(self.f_eta)
...
class StoreLoginScreen(Screen):
    def __init__(self, **kwargs):
        ...
        self.store = JsonStore("store_auth.json")
        self._build()
    def _build(self):
...
class StoreHomeScreen(StoreScreenWithNav):
    ...
    def build_body(self):
        layout = BoxLayout(orientation="vertical")
        header = BoxLayout(size_hint_y=None, height=dp(56), padding=[dp(16), 0], spacing=dp(8)); make_bg(header, C_SURFACE)
        header.add_widget(Label(text="SHV Store", font_size=sp(18), bold=True, color=C_TEAL, halign="left", text_size=(Window.width - dp(80), None)))
...
class AppDetailScreen(Screen):
    ...
    def _build(self, item):
        ...
        if item.get("apk_url", ""):
            ...
            self.action_btn = SHVButton(text="⬇  Download APK", primary=True, height=dp(52))
            self.action_btn.bind(on_release=lambda _: self._handle_button_action(item))
            layout.add_widget(self.action_btn)
            
            # 2. ALWAYS VISIBLE Guide Button
            self.guide_btn = Button(text="View Installation Guide", font_size=sp(12), color=C_TEXT_SEC, background_color=(0,0,0,0), size_hint_y=None, height=dp(36))
            self.guide_btn.bind(on_release=lambda _: self._show_install_guide())
            layout.add_widget(self.guide_btn)
    ...
    def _handle_button_action(self, item):
        if not self.apk_download_path:
    ...
    def _start_download(self, item):
        ...
        app_name = item.get("name", "app").replace(" ", "_")
        app_id = item.get("id", "")
        
        self.status.show("Starting download...", success=True, duration=60)
        self.action_btn.disabled = True 
        
        try: 
            from jnius import autoclass
            Environment = autoclass("android.os.Environment")
            # --- STRAIGHT TO DOWNLOADS FOLDER (NO SUBFOLDER) ---
            dl_dir = Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath()
        except Exception: 
            dl_dir = os.path.join(os.path.expanduser("~"), "Download")
            
        os.makedirs(dl_dir, exist_ok=True)
        dest = os.path.join(dl_dir, f"{app_name}.apk")
        
        def _thread():
            try:
                req = urllib.request.Request(apk_url, headers={"User-Agent": "SHVStore/1.0"})
                    ...
                    with open(dest, "wb") as f:
                        while True:
                            ...
                            if total:
                                ...
                                Clock.schedule_once(lambda _, p=pct: self.status.show(f"Downloading... {p}%", success=True, duration=60))
                
                if app_id: supabase.increment_download(app_id)
                
                def _ui_update(*_):
                    self.apk_download_path = dest
                    self.action_btn.disabled = False
                    self.action_btn.text = "🚀  Install Guide & Open"
                    self.status.show("Download Complete! Ready to install.", success=True, duration=5)
                
                Clock.schedule_once(_ui_update)
            except Exception as e: 
                def _fail(*_):
                    self.action_btn.disabled = False
                    self.status.show(f"Download failed: {str(e)}")
                Clock.schedule_once(_fail)
        
        threading.Thread(target=_thread, daemon=True).start()

    def _open_downloads_folder(self, modal):
        modal.dismiss()
        try:
            ...
            # Disable StrictMode so Android allows us to share raw file:// paths to 3rd party apps
            ...
            # Create a generic VIEW intent pointing to the Downloads folder
            intent = Intent(Intent.ACTION_VIEW)
            uri = Uri.fromFile(File(dl_dir))
            
            # "*/*" acts as a wildcard, forcing Android to ask the user WHICH app they want to use
            intent.setDataAndType(uri, "*/*")
            
            # Force the "Open With..." chooser dialog to appear
            chooser_title = cast("java.lang.CharSequence", String("Open Folder Using..."))
            chooser = Intent.createChooser(intent, chooser_title)
            chooser.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK)
            
            PythonActivity.mActivity.startActivity(chooser)
            self.status.show("Select your File Manager", success=True)
            
        except Exception as e:
            self.status.show(f"Could not open folder.", duration=5)

    def _show_install_guide(self):
        from kivy.uix.modalview import ModalView
        view = ModalView(size_hint=(0.9, 0.88), background_color=(0,0,0,0.8))
        box = BoxLayout(orientation="vertical", padding=dp(20), spacing=dp(12))
        make_bg(box, C_CARD, radius=dp(16))
        
        title = Label(text="Manual Install Guide", font_size=sp(18), bold=True, color=C_TEXT, size_hint_y=None, height=dp(30))
        ...
        guide_text = (
            "[b]Step 0: Download the App[/b]\n"
            "First, ensure you have tapped the 'Download APK' button on the app page to save the file.\n\n"
            "[b]Step 1: Open File Manager[/b]\n"
            "Tap the button below to open your phone's native [color=#00BFA5]Downloads[/color] folder.\n\n"
            "[b]Step 2: Open the APK[/b]\n"
            "Look for the file you just downloaded and tap on it to begin installation.\n\n"
            "[b]Step 3: Allow Unknown Sources[/b]\n"
            "If Android blocks the install, tap 'Settings' on the popup and toggle 'Allow from this source'. Then press back and tap 'Install'.\n\n"
            "[b]Step 4: Google Play Protect Warning[/b]\n"
            "Google might show an 'Unsafe app blocked' warning because this isn't from the Play Store. Tap [color=#EF5350]'More details'[/color] (or the dropdown arrow) and select [color=#00BFA5]'Install anyway'[/color]."
        )
        
        body = Label(text=guide_text, markup=True, font_size=sp(14), color=C_TEXT_SEC, size_hint_y=None, halign="left", valign="top")
        body.bind(width=lambda w, val: setattr(w, 'text_size', (val, None)))
        body.bind(texture_size=lambda w, s: setattr(w, "height", s[1]))
        content.add_widget(body)
        ...
        btn_layout = BoxLayout(orientation="vertical", size_hint_y=None, height=dp(100), spacing=dp(8))
        
        open_btn = SHVButton(text="📂 Open Downloads Folder", primary=True, height=dp(46))
        open_btn.bind(on_release=lambda _: self._open_downloads_folder(view))
        btn_layout.add_widget(open_btn)
        
        close_btn = SHVButton(text="Close", primary=False, height=dp(46))
        close_btn.bind(on_release=view.dismiss)
