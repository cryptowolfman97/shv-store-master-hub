...
class AppDetailScreen(Screen):
    ...
    def _build(self, item):
        ...
        if item.get("apk_url", ""):
            ...
            # --- THE NEW SECONDARY FALLBACK BUTTON ---
            self.fallback_btn = Button(text="Install failed? View manual guide", font_size=sp(12), color=C_TEXT_SEC, background_color=(0,0,0,0), size_hint_y=None, height=0, opacity=0, disabled=True)
            self.fallback_btn.bind(on_release=lambda _: self._show_install_guide())
            layout.add_widget(self.fallback_btn)
    ...
    def _start_download(self, item):
        ...
        try: 
            ...
            dl_dir = os.path.join(Environment.getExternalStoragePublicDirectory(Environment.DIRECTORY_DOWNLOADS).getAbsolutePath(), "SHV_Store")
        except Exception: 
            dl_dir = os.path.join(os.path.expanduser("~"), "Download", "SHV_Store")
        ...
        def _thread():
            try:
                ...
                def _ui_update(*_):
                    ...
                    self.action_btn.text = "🚀  Install App"
                    self.status.show("Download Complete! Ready to install.", success=True, duration=5)
                    # Show the fallback button!
                    self.fallback_btn.height = dp(36)
                    self.fallback_btn.opacity = 1
                    self.fallback_btn.disabled = False
    ...
    def _install_apk(self):
        try:
            ...
            apk_file = File(self.apk_download_path)
            ...
            self.status.show("Launching Installer...", success=True)
            
        except Exception as e:
            self.status.show(f"Install error. Please use manual guide.", duration=10)

    # --- THE NEW MANUAL INSTALLATION GUIDE POPUP ---
    def _show_install_guide(self):
        ...
        guide_text = (
            "If the automatic installer didn't open, follow these steps:\n\n"
            "[b]1. Locate the file:[/b]\n"
            "Open your phone's File Manager app and navigate to:\n"
            f"[color=#00BFA5]{self.apk_download_path}[/color]\n\n"
            "[b]2. Open the APK:[/b]\n"
            "Tap on the downloaded file to begin installation.\n\n"
            "[b]3. Allow Unknown Sources:[/b]\n"
            "If a security prompt appears stating your phone is not allowed to install unknown apps, tap 'Settings'.\n\n"
            "[b]4. Grant Permission:[/b]\n"
            "Toggle the switch to 'Allow from this source'. Press the back button, then tap 'Install'."
        )
        ...
        close_btn = SHVButton(text="OK, Got it!", primary=True, height=dp(48))
        close_btn.bind(on_release=view.dismiss)
        box.add_widget(close_btn)
