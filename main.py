...
class AppDetailScreen(Screen):
    ...
    def _start_download(self, item):
        ...
        # Download to app's private cache — FileProvider serves it from here.
        try:
            from jnius import autoclass
            PythonActivity = autoclass("org.kivy.android.PythonActivity")
            dl_dir = PythonActivity.mActivity.getCacheDir().getAbsolutePath()
        except Exception:
            dl_dir = os.path.join(os.path.expanduser("~"), ".cache")
        ...
        def _thread():
            try:
                ...
                def _ui_update(*_):
                    ...
                    self.status.show("Download complete! Tap Install to continue.", success=True, duration=5)
    ...
    def _delete_apk_file(self, apk_path):
        """Permanently delete the APK. os.remove on Android has no recycle bin."""
        try:
    ...
    def _install_apk(self):
        apk_path = self.apk_download_path
        try:
            from jnius import autoclass
            PythonActivity = autoclass("org.kivy.android.PythonActivity")
            ...
            apk_uri   = FileProvider.getUriForFile(context, authority, File(apk_path))
            ...
            # Permanently delete APK 3 s after installer opens.
            # os.remove on Android is immediate — no recycle bin.
            Clock.schedule_once(lambda *_: self._delete_apk_file(apk_path), 3)

        except Exception as e:
            self.status.show(f"Install error: {str(e)[:80]}", duration=10)
