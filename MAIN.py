...
class StoreNewsCard(SHVCard):
    def __init__(self, item, **kwargs):
        ...
        self.add_widget(SHVLabel(text=item.get("title", ""), font_size=sp(15), bold=True, color=C_TEXT, size_hint_y=None, height=dp(24)))
        body_lbl = Label(text=body[:160] + ("..." if len(body) > 160 else ""), font_size=sp(12), color=C_TEXT_SEC, size_hint_y=None, halign="left", valign="top", text_size=(Window.width - dp(64), None))
        body_lbl.bind(texture_size=lambda w, s: setattr(w, "height", s[1]))
        self.add_widget(body_lbl); self.bind(minimum_height=self.setter("height"))
...
class AppDetailScreen(Screen):
    def on_enter(self):
        self.clear_widgets(); self._build(getattr(App.get_running_app(), "current_app_detail", {}))
    def _build(self, item):
        root = BoxLayout(orientation="vertical"); make_bg(root, C_BG)
        topbar = BoxLayout(size_hint_y=None, height=dp(56), padding=[dp(8), 0], spacing=dp(8)); make_bg(topbar, C_SURFACE)
        back = Button(text="⬅️", font_size=sp(22), color=C_TEAL, background_color=(0,0,0,0), size_hint_x=None, width=dp(40), font_name=FONT_PATH)
        back.bind(on_release=lambda _: setattr(self.manager, "current", "store_catalog"))
        topbar.add_widget(back); topbar.add_widget(Label(text=item.get("name", "App Detail"), font_size=sp(16), bold=True, color=C_TEXT, halign="left", text_size=(Window.width - dp(80), None)))
        root.add_widget(topbar)
        scroll = ScrollView()
        ...
        icon_row.add_widget(info_col); layout.add_widget(icon_row)
        ...
        desc_lbl.bind(texture_size=lambda w, s: setattr(w, "height", s[1])); layout.add_widget(desc_lbl)
        layout.add_widget(Widget(size_hint_y=None, height=dp(8)))
        self.status = StatusBar(size_hint_x=1); layout.add_widget(self.status)
        if item.get("apk_url", ""):
            dl_btn = SHVButton(text="⬇  Download APK", primary=True, height=dp(52))
            dl_btn.bind(on_release=lambda _: self._download_apk(item)); layout.add_widget(dl_btn)
        else: layout.add_widget(SHVLabel(text="APK not yet available", color=C_TEXT_SEC, halign="center", size_hint_y=None, height=dp(40)))
        scroll.add_widget(layout); root.add_widget(scroll); self.add_widget(root)

    def _download_apk(self, item):
        apk_url, app_name, app_id = item.get("apk_url", ""), item.get("name", "app").replace(" ", "_"), item.get("id", "")
        self.status.show("Starting download...", success=True, duration=60)
        def _thread():
            try:
                try: from android.storage import primary_external_storage_path; base = primary_external_storage_path()
                except: base = os.path.expanduser("~")
                dl_dir = os.path.join(base, "Download"); os.makedirs(dl_dir, exist_ok=True); dest = os.path.join(dl_dir, f"{app_name}.apk")
                ...
                with urllib.request.urlopen(req, timeout=120) as resp:
                    ...
                    with open(dest, "wb") as f:
                        while True:
                            ...
                            if total: Clock.schedule_once(lambda _, p=int(downloaded * 100 / total): self.status.show(f"Downloading... {p}%", success=True, duration=60))
                if app_id: supabase.increment_download(app_id)
                def _install(*_):
                    try:
                        from jnius import autoclass
                        Intent = autoclass("android.content.Intent"); Uri = autoclass("android.net.Uri"); File = autoclass("java.io.File"); context = autoclass("org.kivy.android.PythonActivity").mActivity
                        intent = Intent(Intent.ACTION_VIEW); intent.setDataAndType(Uri.fromFile(File(dest)), "application/vnd.android.package-archive"); intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_GRANT_READ_URI_PERMISSION)
                        context.startActivity(intent); self.status.show("Install prompt opened!", success=True)
                    except: self.status.show(f"Saved to Downloads. Open manually to install.")
                Clock.schedule_once(_install)
            except Exception as e: Clock.schedule_once(lambda _, err=str(e): self.status.show(f"Download failed: {err}"))
        threading.Thread(target=_thread, daemon=True).start()
